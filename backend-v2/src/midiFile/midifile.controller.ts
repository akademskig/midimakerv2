...
export class MidiFileController {
  ...
  async createNew(
    @Param('name') name,
    @UploadedFile() midiFile: any,
    @Req() req,
  ) {
    const data = midiFile.buffer.toString('utf-8');
    const midiFileDTO = new MidiFileDto({
      name,
      midiChannels: JSON.parse(data),
    });
  ...
  async updateOne(@Param('id') id, @UploadedFile() midiFile) {
    const data = midiFile.buffer.toString('utf-8');
    return await this.midiFileService.updateOne({
      id,
      midiChannels: JSON.parse(data),
    });
  }
  ...
  async convert(@Res() res, @Param('ext') ext, @Param('id') id) {
    ...
    const filename = await this.midiFileConverter.convertFromMidi(
      midiFile,
      ext,
    );
