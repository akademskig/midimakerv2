...
const initialCtxValue = {
  currentChannel: null,
  setCurrentChannel:
    (value: React.SetStateAction<TChannel | null>) => (value: TChannel) =>
      value,
  audioStateLoading: false,
  setAudioStateLoading:
    (value: React.SetStateAction<boolean>) => (value: boolean) =>
      value,
  channelColor: "#008080",
  setChannelColor: (value: React.SetStateAction<string>) => (value: string) =>
    value,
  noteDuration: 0.125,
  setNoteDuration: (value: React.SetStateAction<number>) => (value: number) =>
    value,
  channels: [],
  setChannels:
    (value: React.SetStateAction<TChannel[] | []>) => (value: TChannel[]) =>
      value,
  notes: [],
  ...
  setNoteRange:
    (value: React.SetStateAction<TNoteRange>) => (value: TNoteRange) =>
      value,
  compositionDuration: 0,
  setCompositionDuration:
    (value: React.SetStateAction<number>) => (value: number) =>
      value,
  resetGrid: () => {},
  updateGridSetup: (channels: TChannel[]) => {},
}

export const AudioStateProviderContext =
  createContext<IAudioStateProviderContext>(initialCtxValue)
const AudioStateProvider = ({
  children,
}: IAudioStateProviderProps): JSX.Element => {
  ...
  const [currentChannel, setCurrentChannel] = useState<TChannel | null>(
    initialChannel
  )
  const [controllerState, setControllerState] = useState<IControllerState>(
    initialControllerState
  )
  const [channels, setChannels] = useState<TChannel[]>([])
  const [compositionDuration, setCompositionDuration] = useState(
    initialCompositionDuration
  )
  ...
  const updateGridSetup = useCallback((channels) => {
    const compositionDuration = channels.reduce(
      (acc: number, curr: TChannel) =>
        curr.duration > acc ? curr.duration : acc,
      0
    )
    const compositionNoteRange = channels.map((channel: TChannel) => {
      return channel.notes.reduce(
        (prev: any, curr: any) => {
          return {
    ...
    const reduced = compositionNoteRange.reduce(
      (prev: any, curr: any) => {
        return {
    ...
    setNoteRange({
      first: Math.min(reduced.min, initialNoteRange.first),
      last: Math.max(reduced.max, initialNoteRange.last),
    })
    setCompositionDuration(
      Math.max(compositionDuration, initialCompositionDuration)
    )
  }, [])
  ...
  const setChannelColor = useCallback(
    (color) => {
      ...
      const newChannels = channels.map((channel: TChannel) =>
        channel.instrumentName === currentChannel?.instrumentName
          ? newChannel
          : channel
      )
