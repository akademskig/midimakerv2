...
import { NotesGridControllerCtx } from "../components/NotesGrid/NotesGridController"
...
const audioContext = new (windowExtended &&
  (windowExtended.AudioContext || windowExtended.webkitAudioContext))()
...
function AudioPlayer(): IAudioPlayer {
  ...
  const { cachedInstruments, currentInstrument } = useContext(
  ...
  const startNote = useCallback(
    (event: any, instrumentName?: string) => {
      const audioNode = instrumentName
        ? cachedInstruments?.[instrumentName]?.play(
            event.midi.toString(),
            0,
            event
          )
        : currentInstrument?.player?.play(event.midi.toString(), 0, event)
      if (audioNode) {
  ...
  const stopNote = useCallback(
    (midiNumber) => {
      ...
      //@ts-ignore
      audioNodes &&
        audioNodes[midiNumber] &&
        audioNodes[midiNumber.toString()]?.stop()
  ...
  const playNote = useCallback(
    (lastEvent: PlayEvent, instrumentName?: string) => {
  ...
  const playAll = useCallback(async () => {
    ...
    const startAndEndTimes = joinedEvents
      .map((event) => (event.time -= timer))
      .filter((t) => t >= 0)
